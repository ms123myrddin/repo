...
module.exports = (bot) => {
  const load = dirs => {
    const commands = readdirSync(`C:/Users/MS314/willow3/src/commands/${dirs}/`).filter(d => d.endsWith('.js')); 
    //commands = all javascript files inside the dirs parameter.

    for (let file of commands) { //for each of these files
      let pull = require(`C:/Users/MS314/willow3/src/commands/${dirs}/${file}`);
  ...
  readdir(`C:/Users/MS314/willow3/src/commands/`, (err, directories) => {
    //directories = the directories the bot commands are separated into
    if (err) logger.error(err); 
    var dirArray = [];
    directories.forEach((f, i) => { 
      if (lstatSync(`C:/Users/MS314/willow3/src/commands/${f}`)) {  
        dirArray.push(f);
      }
    });
